...
def init(DDD):
    ...
    DL_path   = Path(r'C:\Users\USER\Desktop\OCM_202106').joinpath(day)                        # 指定下載的路徑
...
def mergeBYday(day):
    ...
    if not os.path.isfile(nas):
        if os.path.isfile(U) and os.path.isfile(U):
            ...
            ## Add on 2022-04-18
            ## ncap2 -s "time=time-1753152" old.nc new.nc
            ## ncatted -O -a units,time,o,c,"hours since 2000-01-01 00:00:00" new.nc
            subprocess.call("ncap2 -s 'time=time-1753152' {} {}".format(out, new), shell=True)
            subprocess.call("ncatted -O -a units,time,o,c,'hours since 2000-01-01 00:00:00' {}".format(new), shell=True)
